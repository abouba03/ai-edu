...
export async function GET(_req: Request, context: { params: Promise<{ challengeId: string }> }) {
  ...
  try {
    ...
    if (!meta.isPublished) {
      return Response.json({ ok: false, error: 'challenge_not_published' }, { status: 404 });
    }
    ...
    const detail = error instanceof Error ? error.message : 'challenge_load_error';
    return Response.json({ ok: false, error: 'challenge_load_error', detail }, { status: 500 });
  }
}
